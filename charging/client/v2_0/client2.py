...
class ChargePointClient(Cp):
    ...
    def on_certificate_signed(
        ...
        try:
            ...
            reassembled_string = ''.join(cert)
            
            # Convert the hex encoded string back to binary
            der_encoded = bytes.fromhex(reassembled_string)
...
async def launch_client(
    ...
    async with websockets.connect(
            f"ws://{server}:9001/{serial_number}", subprotocols=[Subprotocol("ocpp2.0")]
    ...
    # Open websocket
    async with websockets.connect(
            f"wss://{server}:{port}/{serial_number}", subprotocols=[Subprotocol("ocpp2.0")]
