...
from ocpp.v16 import ChargePoint as Cp, call, call_result, datatypes
...
class ChargePointServer(Cp):
    ...
    act_charge_point_model = ''
    act_charge_point_vendor = ''
    act_charge_box_serial_number = ''
    ...
    def on_boot_notification(
        ...
        self.act_charge_point_model = charge_point_model
        self.act_charge_point_vendor = charge_point_vendor
        self.act_charge_box_serial_number = charge_box_serial_number
    ...
    def on_data_transfer(
        ...
        if message_id == "CertificateExchange":
            print(f"{self.act_charge_box_serial_number}, {self.act_charge_point_model}, {self.act_charge_point_vendor}")
            charging_station = {}
            charging_station["serial_number"] = self.act_charge_box_serial_number
            charging_station["model"] = self.act_charge_point_model
            charging_station["vendor_name"] = self.act_charge_point_vendor
            
            self.act_charge_point_model = ''
            self.act_charge_point_vendor = ''
            self.act_charge_box_serial_number = ''
            charging_station["certificate"] = data
...
async def main():
    ...
    server_test = await websockets.serve(
        on_connect, "fe80::e3a6:46e4:bff9:fb8e%ens33", 9001, subprotocols=[Subprotocol("ocpp1.6")]
    )

    # Start websocket with callback function
    server = await websockets.serve(
        #on_connect, "::", 9000, subprotocols=[Subprotocol("ocpp2.0.1")]
        on_connect_tls, "fe80::e3a6:46e4:bff9:fb8e%ens33", 9000, subprotocols=[Subprotocol("ocpp1.6")], ssl = context
    )
